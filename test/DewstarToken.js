...
describe("DewstarToken contract", function () {
  ...
  describe("Deployment", function () {
    it("Should set the right owner", async function () {
      expect(await DewstarToken.owner()).to.equal(owner.address);
    });

    it("Should assign the total supply of tokens to the owner", async function () {
      const ownerBalance = await DewstarToken.balanceOf(owner.address);
      expect(await DewstarToken.totalSupply()).to.equal(ownerBalance);
    });
  ...
  describe("Transactions", function () {
    it("Should transfer tokens between accounts", async function () {
      ...
      await DewstarToken.transfer(addr1.address, 50);
      const addr1Balance = await DewstarToken.balanceOf(addr1.address);
      expect(addr1Balance).to.equal(50);

      // Transfer 50 tokens from addr1 to addr2
      // We use .connect(signer) to send a transaction from another account
      await DewstarToken.connect(addr1).transfer(addr2.address, 50);
      const addr2Balance = await DewstarToken.balanceOf(addr2.address);
    ...
    it("Should update balances after transfers", async function () {
      ...
      await DewstarToken.transfer(addr1.address, 100);

      // Transfer another 50 tokens from owner to addr2.
      await DewstarToken.transfer(addr2.address, 50);
